...
export default async function handler(req, res) {
  ...
  // ── POST: mensajes entrantes desde Evolution API ─────────────────────────
  if (req.method === 'POST') {
    res.status(200).json({ status: 'ok' })

    try {
      const body = req.body
      // Evolution API envía evento MESSAGES_UPSERT
      ...
      const sendWA = async (to, text) => {
        await fetch(`${EVO_URL}/message/sendText/${instanceName}`, {
          ...
          body: JSON.stringify({ number: to, options: { delay: 500 }, textMessage: { text } })
        })
      }

      // Cargar o crear conversación
      const convRes  = await fetch(`${SUPABASE_URL}/rest/v1/crm_conversations?telefono=eq.%2B${from}&order=created_at.desc&limit=1`, { headers: sbHeaders })
      ...
      // Si modo humano → no responder con IA
      if (conv.mode === 'humano') {
        await fetch(`${SUPABASE_URL}/rest/v1/crm_conversations`, {
          method: 'POST',
          headers: { ...sbHeaders, 'Prefer': 'resolution=merge-duplicates,return=minimal' },
