// api/whatsapp.js — Webhook WhatsApp confiable
// Guarda mensajes y llama al worker de forma esperada. No usa fire-and-forget porque Vercel puede cortar la ejecución.
...
function baseUrl(req) {
  const c = clean(process.env.PUBLIC_BASE_URL || process.env.APP_URL || process.env.CRM_URL)
  if (c) return c.replace(/\/$/, '')
  const host = req.headers['x-forwarded-host'] || req.headers.host
  const proto = req.headers['x-forwarded-proto'] || 'https'
  return host ? `${proto}://${host}` : 'https://crm.rabbittscapital.com'
}
function eventName(b = {}) { return clean(b.event || b.type || b.eventName || b.action || '').toLowerCase().replace(/_/g, '.') }
function unwrap(o = {}) {
...
function jidOf(o = {}) { return clean(o?.key?.remoteJid || o?.remoteJid || o?.jid || o?.from || o?.sender || o?.participant || '') }
...
function messages(b = {}) {
  const d = b.data, a = []
  if (Array.isArray(d)) a.push(...d)
  else if (Array.isArray(d?.messages)) a.push(...d.messages)
  else if (Array.isArray(d?.message)) a.push(...d.message)
  else if (d?.key || d?.message || d?.remoteJid || d?.sender) a.push(d)
  else if (b?.key || b?.message || b?.remoteJid || b?.sender) a.push(b)
  return a.filter(Boolean)
}
async function readSetting(db, key, fallback = null) { try { const { data } = await db.from('crm_settings').select('value').eq('key', key).single(); return data?.value ?? fallback } catch { return fallback } }
...
async function findOrCreateConv(db, { tel, phone, name, text, instance }) {
  let found = []
  try { const { data } = await db.from('crm_conversations').select('*').eq('telefono', tel).order('updated_at', { ascending: false }).limit(10); found = data || [] } catch {}
  const conv = found.find(c => c.last_message && c.last_message !== '[mensaje]' && c.last_message !== '[mensaje multimedia]') || found[0]
  if (conv) {
    ...
    try { const { data } = await db.from('crm_conversations').update(upd).eq('id', conv.id).select().single(); return data || { ...conv, ...upd } } catch { return { ...conv, ...upd } }
  }
  const base = { id: phone ? `wa-${phone}` : makeId('wa'), telefono: tel, nombre: name || tel || phone, mode: 'ia', status: 'activo', last_message: text || '[mensaje]', lead_id: null, created_at: nowIso(), updated_at: nowIso() }
  for (const p of [{ ...base, instanceName: instance || null, origen: 'whatsapp' }, { ...base, instanceName: instance || null }, base]) {
    try { const { data, error } = await db.from('crm_conversations').insert(p).select().single(); if (!error && data) return data } catch {}
  }
  return base
}
...
async function duplicate(db, convId, content) {
  ...
  try { const { data, error } = await db.from('crm_conv_messages').select('id').eq('conv_id', convId).eq('role', 'user').eq('content', content).gte('created_at', since).limit(1); return !error && data?.length > 0 } catch { return false }
}
async function ensureLead(db, conv, { tel, phone, name, text }) {
  try { const { data } = await db.from('crm_leads').select('*').eq('telefono', tel).limit(1); if (data?.length) { if (conv?.id && !conv.lead_id) await db.from('crm_conversations').update({ lead_id: data[0].id }).eq('id', conv.id); return data[0] } } catch {}
  ...
  for (const p of [{ ...base, fuente: 'whatsapp', origen: 'whatsapp' }, { ...base, fuente: 'whatsapp' }, base]) {
    try { const { data, error } = await db.from('crm_leads').insert(p).select().single(); if (!error && data) { if (conv?.id) await db.from('crm_conversations').update({ lead_id: data.id }).eq('id', conv.id); return data } } catch {}
  }
  return null
}
async function defaultInstance(db, fallback = '') {
  ...
  if (Array.isArray(nums)) { const n = nums.find(x => x.activo !== false && x.instanceName) || nums.find(x => x.instanceName); if (n?.instanceName) return n.instanceName }
...
async function sendWa({ instance, number, text }) {
  ...
  if (!url || !key || !instance || !number || !text) return { ok: false }
...
async function direct(req, res, db, b) {
  const to = clean(b.to || b.number || b.phone), text = clean(b.mensaje || b.message || b.text), inst = await defaultInstance(db, clean(b.instance || b.instanceName))
  const result = await sendWa({ instance: inst, number: to, text })
  return res.status(200).json({ ok: result.ok, sent: result.ok, result })
}
async function qrOrConn(db, b, event, instance) {
...
async function triggerWorker(req, payload) {
  const url = `${baseUrl(req)}/api/agent-worker`
  try {
    ...
    console.error('[WA worker trigger]', e.message)
    return { ok: false, error: e.message }
  }
}

export default async function handler(req, res) {
  if (req.method === 'GET') return res.status(200).json({ ok: true, endpoint: 'api/whatsapp', mode: 'reliable-worker-await' })
  if (req.method !== 'POST') return res.status(405).json({ ok: false })
  const db = sb()
  if (!db) return res.status(200).json({ ok: false, error: 'supabase_env_missing' })
  const b = req.body || {}
  if ((b.to || b.number || b.phone) && (b.mensaje || b.message || b.text) && !b.event && !b.data?.key) return direct(req, res, db, b)
  const event = eventName(b)
  const raw = messages(b)
  const instance = clean(b.instance || b.instanceName || b.instance_name || b.data?.instance || b.data?.instanceName || raw[0]?.instance || raw[0]?.instanceName || '')
  const h = await qrOrConn(db, b, event, instance)
  if (h && !raw.length) return res.status(200).json(h)
  if (!(event.includes('message') || raw.length)) return res.status(200).json({ ok: true, skipped: event || 'not_message' })

  const results = []
  for (const m of raw) {
    const jid = jidOf(m), phone = phoneOf(jid), tel = telOf(phone), text = textOf(m), name = nameOf(m), fromMe = m?.key?.fromMe === true || m?.fromMe === true, id = msgId(m)
    ...
    if (fromMe) {
      await saveMsg(db, conv.id, 'assistant', text, { manual: true })
    ...
    if (ia.activo === false || conv.mode === 'humano') { results.push({ ok: true, skipped: ia.activo === false ? 'ia_off' : 'human_mode', convId: conv.id }); continue }
    const worker = await triggerWorker(req, { convId: conv.id, phone, tel, instance, messageId: id, ts: Date.now() })
    results.push({ ok: true, queued: worker.ok, worker, convId: conv.id })
  }
  return res.status(200).json({ ok: true, queued: true, results })
}
