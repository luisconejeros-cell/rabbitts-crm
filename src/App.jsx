...
function CerebroRabito({ supabase, dbReady, iaConfig, upd }) {
  ...
  const readFileText = async (file) => {
    ...
    // PDF y Word: Claude los lee nativamente
    if (name.endsWith('.pdf') || name.endsWith('.docx') || name.endsWith('.doc')) {
      ...
      if (!response.ok || data.error) throw new Error(data?.error?.message || 'Error leyendo documento')
...
function CerebroRabito({ supabase, dbReady, iaConfig, upd }) {
  ...
  const uploadFiles = async (files) => {
    ...
    for (const file of files) {
      try {
        ...
        if (clean.length < 10) { setMsg({ type:'error', text:`${file.name}: archivo vacío o no legible` }); continue }
        newDocs.push({
          id: 'doc-' + Date.now() + '-' + Math.random().toString(36).slice(2,5),
          nombre: file.name,
          content: clean.slice(0, 15000), // max 15k chars por doc
          chars: clean.length,
          truncado: clean.length > 15000,
          fecha: new Date().toISOString()
        })
      } catch(e) { setMsg({ type:'error', text:`Error leyendo ${file.name}` }) }
    }
    if (newDocs.length) {
      const allDocs = [...docs, ...newDocs]
      setDocs(allDocs)
      // Guardar en Supabase como drive_content para que agent.js lo lea
      const driveContent = {
  ...
  const deleteDoc = async (docId) => {
    ...
    if (dbReady) {
      await supabase.from('crm_settings').upsert({ key: 'drive_content', value: driveContent })
      upd(['cerebroDocs'], updated)
      upd(['driveConectado'], updated.length > 0)
    }
    setMsg({ type:'success', text:'Documento eliminado' })
  }

  const totalChars = docs.reduce((s,d) => s + (d.chars||d.content?.length||0), 0)
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div style={{background:B.light,border:'1px solid '+B.border,borderRadius:12,padding:'16px'}}>
        ...
        <div style={{fontSize:12,color:'#6b7280',padding:'8px 12px',background:'#fff',borderRadius:8,border:'1px solid #e5e7eb'}}>
          💡 <strong>Qué subir:</strong> Personalidad del agente, guión de ventas, proyectos con precios, preguntas frecuentes. Formatos soportados: <strong>.pdf, .docx, .doc, .txt, .md, .csv</strong>
        </div>
      </div>
      ...
      >
        ...
        <div style={{fontWeight:700,fontSize:14,color:B.primary,marginBottom:4}}>
          {uploading ? '⏳ Cargando...' : 'Haz clic o arrastra archivos aquí'}
        </div>
        <div style={{fontSize:12,color:B.mid}}>.pdf, .docx, .doc, .txt, .md, .csv — hasta 15.000 caracteres por archivo</div>
        <input ref={fileRef} type="file" multiple accept=".txt,.md,.csv,.html,.text,.pdf,.docx,.doc"
          style={{display:'none'}}
      ...
      {/* Status message */}
      {msg && (
        <div style={{padding:'10px 14px',borderRadius:8,background:msg.type==='error'?'#FEF2F2':'#DCFCE7',
          color:msg.type==='error'?'#991b1b':'#14532d',fontSize:13,fontWeight:600}}>
          {msg.text}
        </div>
      )}
      ...
      {docs.length > 0 && (
        <div style={{background:'#fff',border:'1px solid #E2E8F0',borderRadius:12,padding:'16px'}}>
          <div style={{fontWeight:700,fontSize:13,color:B.primary,marginBottom:12}}>
            📚 Documentos en el cerebro de Rabito ({docs.length})
          </div>
          {docs.map(doc => (
            <div key={doc.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:8,border:'1px solid #f0f4ff',marginBottom:6,background:'#f9fbff'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:600,fontSize:13,color:'#0F172A',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{doc.nombre}</div>
                <div style={{fontSize:11,color:B.mid}}>
                  {((doc.chars||doc.content?.length||0)/1000).toFixed(1)}k caracteres
                  {doc.truncado && ' (truncado a 15k)'}
                  {doc.truncado && <span style={{color:'#f59e0b',fontWeight:600}}> ⚠️</span>}
